    ...
    */
    if (!function_exists('getFormattedNumber')) {
        function getFormattedNumber(
            ...
            // Example usage of PhpSpreadsheet's number formatting
            $numberFormat = "#,##0"; // Default format
            if ($precision > 0) {
                $numberFormat .= '.' . str_repeat('0', $precision);
            }
            if ($groupingUsed) {
                $numberFormat .= '_);(#,##0';
            }
            return number_format($value, $precision, '.', ','); // Return formatted number
        }
    }
